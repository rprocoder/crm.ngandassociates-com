...
const BasicForm = () => {
  ...
  const [servicename, setServicename] = useState("Trademark Objection Reply");
...
const PersonalForm = () => {
  const [name, setName] = useState("Trademark Objection Reply");
...
const SummaryForm = () => {
  const [servicename] = useState('Trademark Objection Reply');
  const [documents, setDocuments] = useState([]);
  let token = localStorage.getItem("access_token");
  const fetchCompanydocuments = async () => {
    const result = await axios.get(
