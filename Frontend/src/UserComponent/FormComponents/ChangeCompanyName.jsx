...
const BasicForm = () => {
  ...
  const [servicename, setServicename] = useState("Change Company Name");
...
const PersonalForm = () => {
  const [name, setName] = useState("Change Company Name");
...
const SummaryForm = () => {
  const [servicename] = useState('Change Company Name');
  const [documents, setDocuments] = useState([]);
  let token = localStorage.getItem("access_token");
  const fetchCompanydocuments = async () => {
    const result = await axios.get(
