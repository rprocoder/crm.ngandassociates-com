...
const BasicForm = () => {
  ...
  const [servicename, setServicename] = useState("Trademark Opposition");
...
const PersonalForm = () => {
  const [name, setName] = useState("Trademark Opposition");
...
const SummaryForm = () => {
  const [servicename] = useState('Trademark Opposition');
  const [documents, setDocuments] = useState([]);
  let token = localStorage.getItem("access_token");
  const fetchCompanydocuments = async () => {
    const result = await axios.get(
