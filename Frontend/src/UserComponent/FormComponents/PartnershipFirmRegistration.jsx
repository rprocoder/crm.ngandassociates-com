...
const BasicForm = () => {
  ...
  const [servicename, setServicename] = useState("Partnership Firm Registration");
...
const PersonalForm = () => {
  const [name, setName] = useState("Partnership Firm Registration");
...
const SummaryForm = () => {
  const [servicename] = useState('Partnership Firm Registration');
  const [documents, setDocuments] = useState([]);
  let token = localStorage.getItem("access_token");
  const fetchCompanydocuments = async () => {
    const result = await axios.get(
