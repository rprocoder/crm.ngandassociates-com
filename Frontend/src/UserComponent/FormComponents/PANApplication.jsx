...
const BasicForm = () => {
  ...
  const [servicename, setServicename] = useState("PAN Application");
...
const PersonalForm = () => {
  const [name, setName] = useState("PAN Application");
...
const SummaryForm = () => {
  const [servicename] = useState('PAN Application');
  const [documents, setDocuments] = useState([]);
  let token = localStorage.getItem("access_token");
  const fetchCompanydocuments = async () => {
    const result = await axios.get(
