...
const BasicForm = () => {
  ...
  const [servicename, setServicename] = useState("Close a Private Limited Company");
...
const SummaryForm = () => {
  const [servicename] = useState('Close a Private Limited Company');
  const [documents, setDocuments] = useState([]);
  let token = localStorage.getItem("access_token");
  const fetchCompanydocuments = async () => {
    const result = await axios.get(
...
const CloseaPrivateLimitedCompany = () => {
  const classes = useStyles();
