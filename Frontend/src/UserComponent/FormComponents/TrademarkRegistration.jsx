...
const BasicForm = () => {
  ...
  const [servicename, setServicename] = useState("Trademark Registration");
...
const PersonalForm = () => {
  const [name, setName] = useState("Trademark Registration");
...
const SummaryForm = () => {
  const [servicename] = useState('Trademark Registration');
  const [documents, setDocuments] = useState([]);
  let token = localStorage.getItem("access_token");
  const fetchCompanydocuments = async () => {
    const result = await axios.get(
