...
const BasicForm = () => {
  ...
  const [servicename, setServicename] = useState("Ayush License");
...
const PersonalForm = () => {
  const [name, setName] = useState("Ayush License");
...
const SummaryForm = () => {
  const [servicename] = useState('Ayush License');
  const [documents, setDocuments] = useState([]);
  let token = localStorage.getItem("access_token");
  const fetchCompanydocuments = async () => {
    const result = await axios.get(
