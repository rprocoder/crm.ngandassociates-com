...
const BasicForm = () => {
  ...
  const [servicename, setServicename] = useState("Close a Limited Liability Partnership");
  ...
  const addOrderrequirements1 = async () => {
    ...
    await axios({
      method: "post",
      url: "http://admin.ngandassociates.com/orderrequirements/",
      data: formField,
    }).then((response) => {
...
const PersonalForm = () => {
  const [name, setName] = useState("Close a Limited Liability Partnership");
      ...
      >
        ...
        <div className="payment">
          <div className="form-group">
            ...
            >
              <option>Close a Limited Liability Partnership</option>
            </select>
          </div>
          <div className="form-group">
...
const SummaryForm = () => {
  const [servicename] = useState('Close a Limited Liability Partnership');
  const [documents, setDocuments] = useState([]);
  let token = localStorage.getItem("access_token");
  const fetchCompanydocuments = async () => {
    const result = await axios.get(
...
const CloseaLimitedLiabilityPartnership = () => {
  const classes = useStyles();
