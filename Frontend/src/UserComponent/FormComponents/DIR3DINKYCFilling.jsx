...
const BasicForm = () => {
  ...
  const [servicename, setServicename] = useState("DIR3 DIN KYC Filling ");
...
const PersonalForm = () => {
  const [name, setName] = useState("DIR 3 DIN KYC Filling");
      ...
      >
        ...
        <div className="payment">
          <div className="form-group">
            ...
            >
              <option> DIR 3 DIN KYC Filling</option>
            </select>
          </div>
          <div className="form-group">
...
const SummaryForm = () => {
  const [servicename] = useState('DIR3 DIN KYC Filling ');
  const [documents, setDocuments] = useState([]);
  let token = localStorage.getItem("access_token");
  const fetchCompanydocuments = async () => {
    const result = await axios.get(
...
export default DIR3DINKYCFilling;