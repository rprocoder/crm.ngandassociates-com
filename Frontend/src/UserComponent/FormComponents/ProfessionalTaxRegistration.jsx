...
const BasicForm = () => {
  ...
  const [servicename, setServicename] = useState("Professional Tax Registration");
...
const PersonalForm = () => {
  const [name, setName] = useState("Professional Tax Registration");
...
const SummaryForm = () => {
  const [servicename] = useState('Professional Tax Registration');
  const [documents, setDocuments] = useState([]);
  let token = localStorage.getItem("access_token");
  const fetchCompanydocuments = async () => {
    const result = await axios.get(
