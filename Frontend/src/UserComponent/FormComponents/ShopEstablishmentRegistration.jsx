...
const BasicForm = () => {
  ...
  const [servicename, setServicename] = useState("Shop Establishment Registration");
...
const PersonalForm = () => {
  const [name, setName] = useState("Shop Establishment Registration");
...
const SummaryForm = () => {
  const [servicename] = useState('Shop Establishment Registration');
  const [documents, setDocuments] = useState([]);
  let token = localStorage.getItem("access_token");
  const fetchCompanydocuments = async () => {
    const result = await axios.get(
...
const ShopEstablishmentRegistration = () => {
  const classes = useStyles();
