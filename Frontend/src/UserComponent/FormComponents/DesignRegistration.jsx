...
const BasicForm = () => {
  ...
  const [servicename, setServicename] = useState("Design Registration");
...
const PersonalForm = () => {
  const [name, setName] = useState("Design Registration");
...
const SummaryForm = () => {
  const [servicename] = useState('Design Registration');
  const [documents, setDocuments] = useState([]);
  let token = localStorage.getItem("access_token");
  const fetchCompanydocuments = async () => {
    const result = await axios.get(
