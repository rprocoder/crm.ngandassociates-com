...
const BasicForm = () => {
  ...
  const [servicename, setServicename] = useState("Section 8  Company Registration");
...
const PersonalForm = () => {
  const [name, setName] = useState("Section 8 Company Registration");
...
const SummaryForm = () => {
  const [servicename] = useState('Section 8  Company Registration');
  const [documents, setDocuments] = useState([]);
  let token = localStorage.getItem("access_token");
  const fetchCompanydocuments = async () => {
    const result = await axios.get(
