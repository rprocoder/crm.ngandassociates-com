...
const BasicForm = () => {
  ...
  const [servicename, setServicename] = useState("Startup India Registration");
...
const PersonalForm = () => {
  const [name, setName] = useState("Startup India Registration ");
...
const SummaryForm = () => {
  const [servicename] = useState('Startup India Registration');
  const [documents, setDocuments] = useState([]);
  let token = localStorage.getItem("access_token");
  const fetchCompanydocuments = async () => {
    const result = await axios.get(
