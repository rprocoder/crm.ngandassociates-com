...
const BasicForm = () => {
  ...
  const [servicename, setServicename] = useState("Limited Liability Partnership Registration");
...
const PersonalForm = () => {
  const [name, setName] = useState("Limited Liability Partnership Registration");
...
const SummaryForm = () => {
  const [servicename] = useState('Limited Liability Partnership Registration');
  const [documents, setDocuments] = useState([]);
  let token = localStorage.getItem("access_token");
  const fetchCompanydocuments = async () => {
    const result = await axios.get(
