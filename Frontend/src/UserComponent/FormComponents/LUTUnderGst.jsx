...
const BasicForm = () => {
  ...
  const [servicename, setServicename] = useState("LUT Under GSt");
...
const PersonalForm = () => {
  const [name, setName] = useState("LUT Under Gst");
...
const SummaryForm = () => {
  const [servicename] = useState('LUT Under GSt');
  let token = localStorage.getItem("access_token");
  const [documents, setDocuments] = useState([]);
  const fetchCompanydocuments = async () => {
    const result = await axios.get(
