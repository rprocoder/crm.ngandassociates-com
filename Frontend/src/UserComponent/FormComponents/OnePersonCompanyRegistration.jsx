...
const BasicForm = () => {
  ...
  const [servicename, setServicename] = useState("One Person Company Registration");
...
const PersonalForm = () => {
  const [name, setName] = useState("One Person Company Registration");
...
const SummaryForm = () => {
  const [servicename] = useState('One Person Company Registration');
  const [documents, setDocuments] = useState([]);
  let token = localStorage.getItem("access_token");
  const fetchCompanydocuments = async () => {
    const result = await axios.get(
